...
class StationServerClient(QObject):
    ...
    def _sendConnect(self) -> None:
        message = ConnectMessage(sender_type=SenderType.GAS_NOZZLE)
        self._client.sendTextMessage(message.to_json())

    def sendUseStationT2(self) -> None:
        message = UseStationT2Message()
        self._client.sendTextMessage(message.to_json())

    def sendUseMobileAppT2(self) -> None:
        message = UseMobileAppT2Message()
        self._client.sendTextMessage(message.to_json())
